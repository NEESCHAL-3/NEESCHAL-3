...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=15&duration=2000&pause=3000&color=DC143C&center=true&vCenter=true&width=780&lines=Learning%2C+experimenting%2C+and+exploring;Turning+caffeine+into+segmentation+faults+since+day+one;Works+perfectly+until+someone+watches+me+run+it;The+comment+says+don%27t+touch+this+so+obviously+I+touched+it;Sleep+is+just+a+RAM+refresh+for+humans;Studying+CSE+is+10%25+coding+and+90%25+deciphering+compiler+errors;If+it+works+on+my+machine%2C+then+we+ship+my+machine;Permission+denied+is+the+real+final+boss;The+code+works%2C+I+don%27t+know+why%2C+please+don%27t+touch+it;Coffee.exe+stopped+responding%2C+restarting+human...;rm+-rf+out/+and+pretending+I+didn%27t+lose+2+hours;Spent+6+hours+debugging+and+forgot+to+save+the+file;My+code+has+no+bugs%2C+only+undocumented+features;Reading+logs+like+ancient+Egyptian+hieroglyphics;Added+a+debug+print%2C+it+fixed+itself%2C+the+print+stays+forever;My+terminal+history+is+basically+a+horror+story;The+build+failed+because+I+looked+at+it+wrong;The+compiler+error+is+just+a+puzzle+written+by+Satan;WSL2+politely+consuming+31.9+GB+of+my+32+GB+RAM;Windows+Update+restarting+at+99%25+during+unsaved+builds;Bootloop+is+just+the+device+meditating+really+hard;There+is+no+cloud%2C+just+someone+else%27s+computer+on+fire;Gentoo+users+compiling+Firefox+to+heat+up+the+room+in+winter;Dijkstra+at+4+AM+is+the+shortest+path+between+me+and+caffeine;One+wrong+command+and+my+entire+weekend+disappeared;Debugging+at+3+AM+hits+different;My+code+works+and+I+don%27t+know+why%2C+peak+impostor+syndrome;void+***ptr+entered+the+chat+and+my+sanity+vanished;Commenting+out+failing+tests+so+the+CI+pipeline+turns+green;Turning+temporary+workaround+into+a+permanent+architecture;A+full+day+of+coding+can+save+15+minutes+of+reading+docs;Quiet+commits%2C+loud+results;Said+it+was+a+small+fix%2C+git+diff+showed+47+files+changed;Changing+variable+names+because+the+bug+scared+me" alt="Typing SVG" />
